...
int SQLiteDataRow::GetFieldCount() {
    //DoMsgLine("GetFieldCount");
    return m_fCount;
}

int SQLiteDataRow::GetFieldIndexOf(LPCSTR szFieldName) {
    //DoMsgLine("GetFieldIndexOf FieldName = %s",mmNone,szFieldName);
...
BOOL SQLiteDataProvider::RefreshRow(CDataRow* pRowFrom) {
    //if(m_debug)
    //    DoMsgLine("Поставщик данных SQLite: обновить строку");
    if(!pRowFrom){
        return FALSE;
    }
...
BOOL SQLiteDataProvider::GetRowCount(UINT* pCount) {
    ...
    return FALSE;
    //DoMsgLine("GetRowCount");
    SQLiteQuery* pQuery = getQuery((qTypes)qCount);
    //return FALSE;
    ...
    SQLiteDataRow* pDataRow = tempDst[0];
    CValue value = static_cast<SQLiteDataRow*>(pDataRow)->m_pValues[0];
    unsigned int val = (unsigned)(int)value.m_Number.GetDouble();
    //return FALSE;
...
BOOL SQLiteDataProvider::SetQueryText(CValue** ppParams) {
    if(!m_pDataBase)
        CBLModule::RaiseExtRuntimeError("Не установлена база данных", FALSE);
    ...
    int newIDField = -1;
    {
        ...
        for(int idx = 0; idx < fieldsCount; idx++)
        ...
        for(idx = 0; idx < strKeyFields.GetSize(); idx++) {
            ...
            for(int k = 0; k < newKeyFields.GetSize(); k++) {
    ...
    if(!pQuery) {
        ...
        if(m_debug) {
            static const LPCSTR names[] = {
                "следющие записи" //qDown
                ,"предыдущие записи" // qUp
                ,"первые записи" // qTop
                ,"последние записи" //qBottom
                ,"запись по id" //qID
                ,"обновление строки" //qCurRow
                ,"быстрый поиск" //qQuickSearch
                ,"количество строк" //qCount
                ,"последние записи" //qLast
                ,"непонятно что"
            };
            //enum qTypes {qDown, qUp, qTop, qBottom, qID, qCurRow, qQuickSearch,qCount,qLast};
            DoMsgLine("Поставщик данных SQLite: Формирование запроса '%s'", mmNone, names[type]);
        }
        // Формируем текст запроса
        if(m_select.IsEmpty()) {	// Надо перестроить список полей
            //DoMsgLine("m_select.IsEmpty");
            if(type == qCount) {
                DoMsgLine("m_queries[type], type == qCount");
            }
            ...
            //DoMsgLine("m_select.NotEmpty()");

            //if(type == qCount) {
                //m_select.Empty();
            //    m_select="select count(*)\r\n";
            //}
        ...
        */
        CString mainSqlQuery, strSql;
        if (type == qCount) {
            mainSqlQuery="select count (*)\r\n"+m_from;
        }else{
            mainSqlQuery=m_select+m_from;
        }
        ...
        if(qDown == type || qUp == type) {
            ...
            strSql = mainSqlQuery;
            // Djelf if(qID == type) вылетает при idFieldPos()=-1
            if(qID == type && idFieldPos() != -1 ) {
                strSql += " where\r\n";
                if(!m_where.IsEmpty())
                    strSql += m_where + "\r\n and \r\n";
...
void SQLiteDataProvider::setColumnsChanged() {
    ...
    m_sqlParams.dirtyQueries = 0xFFFFFFFF;
    // а вот этого не требуется! управлением составом колонок занимается ТабличноеПоле
    //OnFieldsChanged();
}
