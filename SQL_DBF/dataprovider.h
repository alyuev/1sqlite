...
class SQLiteDataProvider : public CContextImpl<SQLiteDataProvider>
  ...
  public:
    ...
    int fieldsCount() const				{
        return m_countOfColumns;
    }
    LPCSTR fieldName(int iIndex)const	{
        return m_queryFieldsNames[iIndex];
    }
    ...
    BL_DEFVAL_FOR(SetDataBase) {
        if(pValue) pValue->Reset();
        return TRUE;
    }
    ...
    BL_END_CONTEXT();
  private:
    enum qTypes {qDown, qUp,qTop,qBottom,qID,qCurRow,qQuickSearch,qCount,qLast}; //qLast должен быть последним!
    ...
    int fieldsInfoCount() const {
        return m_fieldsInfoCount;
    }
    const field_info&	fieldInfo(DWORD idx) const {
        return m_fieldsInfo[idx];
    }
    int keyFieldsCount() const				{
        return m_keyFieldCount;
    }
    const CString& keySQLParamName(DWORD idx) const {
        return m_keyParamNames[idx];
    }
    int idFieldPos() const {
        return m_idFieldPos;
    }
    ...
    int		   		    m_fieldsInfoCount;	// Количество всех полей запроса
    int			    	m_keyFieldCount;	// Количество ключевых полей
    int     			m_idFieldPos;		// Номер ид-поля в списке всех полей
    int			       	m_countOfColumns;	// Количество колонок в текущем запросе
    BOOL				m_debug;			// Флаг отладки
    field_info*			m_lastQSColumn;		// Для которого поля подготовлен запрос быстрого поиска
    int		    		m_QSColumn;			// Номер колонки в запросе быстрого поиска, в котором искать
    ...
    BOOL				m_bReverseOrder;	// Флаг обратной сортировки
};

class SimpleResultLoader :  public CBLContext, public ISQLiteResultLoader {
  public:
    virtual int init(int colCountInResult, sqlite3* pBase, CValue* pParam) {
        return m_count = colCountInResult;
    }
    virtual void setColumn(int colIdx, const CString& colName, const CType& type) {}
