...
struct ValueStrWork {
    inline static void str2val(CValue* pValue, LPCSTR str, Types1C type = typeUndefined, long mdID = 0) {
        ...
        if (type==typeUndefined){
            DoMsgLine("typeUndefined %s",mmNone,str);
        }
        else{
            DoMsgLine("typeDefined %s",mmNone,str);
        }
        */
...
template<typename T>
inline static void bindBlob(T& binder, const CValue* pValue, int mod) {
    binder.bindBlob(pValue);
}
...
static void bindValue(T& binder, const CValue* pValue, int mod) {
    ...
    case typeUndefined:
        ...
        break;
    case typeNumber:
        // не тут инт64 DoMsgLine("bindValue bindNumeric %s",mmNone,pValue->GetString());
    ...
    default:
        if(pValue->type == 100) {
            if(strcmp(pValue->GetTypeString(), "BinaryData") == 0) {
                //DoMsgLine("bindValue pValue->type = %i %s", mmNone, pValue->type, pValue->GetTypeString());
                bindBlob(binder, pValue, mod);
                return;
            } else {
                char buf[100];
                sprintf(buf, "Неизвестный тип значения: %s", pValue->GetTypeString());
                binder.error(buf);
                return;
            }
        } else {
