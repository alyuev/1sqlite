...
struct field_info {
    ...
    field_cmp_type type() const {
        return (field_cmp_type)t;
    }
    DWORD length() const {
        return l;
    }
    DWORD precession()		const {
        return p;
    }
    DWORD offsetInRecord()	const {
        return o;
    }
...
struct index_info {
    DWORD fieldsCount()		const {
        return c;
    }
    DWORD keySize()			const {
        return s;
    }
    const idx_field_info* fields() const {
        return fi;
    }
...
struct phisical_info {
    ...
    const field_info& __fastcall field(int pos) const {
        return m_pFields[pos];
    }
    const index_info* __fastcall index(int pos) const {
        return m_ppIndexes[pos];
    }
    ...
    const char* tableReadBufer() const {
        return pTableBuf;
    }
    ...
    fld2idxIt __fastcall fld2idx(DWORD fld)	const {
  ...
  private:
    ...
    static phisical_info* __fastcall buildInfo(CTableEx* pTable);
