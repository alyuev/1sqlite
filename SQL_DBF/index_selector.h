...
struct index_selector {
    ...
    const idx_node* __fastcall bestIndex(sqlite3_index_info* pIdx, const CVtabInfo& table) {
        constraint_ptr pCtr = pIdx->aConstraint;
        usage_ptr pUsg = pIdx->aConstraintUsage;
        //DoMsgLine("bestIndex %s",mmNone,table.tableName());
    ...
    static TestOp __fastcall sqlite2TestOp(DWORD op) {
  ...
  private:
    ...
    idx_node* __fastcall find_idx(DWORD idxNum, const CVtabInfo& table, BOOL bCreate) {
    ...
    idx_field_node* __fastcall find_pos_in_idx(idx_node* pIdx, DWORD posInIndex) {
    ...
    void __fastcall addConstaraint(DWORD idxNum, DWORD posInIdx, TestOp op, usage_ptr pUsage, const CVtabInfo& table) {
    ...
    BOOL __fastcall addOrderFieldInIdx(DWORD idxNum, DWORD posInIdx, DWORD posInOrderBy, NeedOrderOutput order, const CVtabInfo& table) {
    ...
    BOOL __fastcall addOrderFieldToIndexes(DWORD fieldNumInTable, DWORD posInOrderBy, NeedOrderOutput order, const CVtabInfo& table) {
        BOOL ret = FALSE;
    ...
    void __fastcall processOrderBy(orderby_ptr pOrderBy, int countOrderBy, const CVtabInfo& table) {
        if(!countOrderBy)
            return;
    ...
    DWORD __fastcall indexScores(idx_node* idx) {
    ...
    idx_node* __fastcall bestOfTheBest() {
        DWORD maxScores = 0;
        idx_node* pBest = NULL;
