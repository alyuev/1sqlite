...
struct testConstraint {
    ...
    static BOOL __fastcall doTest(testConstraint* ptr, const cursor_data& cursor) {
        while(ptr) {
            if(!ptr->test(cursor))
                return FALSE;
            ptr = ptr->next;
    ...
    testConstraint(testConstraint*& p) : next(p) {
        p = this;
    }
    static void __fastcall append(testConstraint*& ret, testConstraint*& n1, testConstraint*& n2) {
        ret = n1;
        n1->next = n2;
        n2 = NULL;
        n1 = NULL;
...
class FilterMachine {
  public:
    ...
    select_iterator* __fastcall process(const unsigned char* pByteCode, sqlite3_value** args);

    static void __fastcall build(const class CVtabInfo& table, sqlite3_index_info* pIdx, const struct idx_node* pBest);

    static class select_iterator* __fastcall parseFilter(struct cursor_data&, const class CVtabInfo&, const char*, sqlite3_value**);
  protected:

    enum keySrc {keyFKEq, keyFKnEq, keyPKEq, keyPKnEq};

    select_iterator* __fastcall GotoFirstToLast();
    select_iterator* __fastcall GotoFirstToMax();
    select_iterator* __fastcall GotoMinToLast();
    select_iterator* __fastcall GotoMinToMax();
    select_iterator* __fastcall GotoLastToFirst();
    select_iterator* __fastcall GotoMaxToFirst();
    select_iterator* __fastcall GotoLastToMin();
    select_iterator* __fastcall GotoMaxToMin();
    select_iterator* __fastcall GotoToRow();
