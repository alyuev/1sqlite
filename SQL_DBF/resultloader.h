...
class ISQLiteResultLoader {
  public:
    ...
    virtual int init(int colCountInResult, sqlite3* pBase, CValue* pParam) = 0;
    // Добавление колонки в результат запроса
    virtual void setColumn(int colIdx, const CString& colName, const CType& type) = 0;
...
// Загрузчик в ТаблицуЗначений
class IVTResultLoader : public ISQLiteResultLoader {
  public:
    IVTResultLoader() : m_pCont(NULL) {}
    ~IVTResultLoader() {
        if(m_pCont) m_pCont->DecrRef();
    }

    // Подготовка приемника к приему результатов
    virtual int init(int colCountInResult, sqlite3*, CValue* pClear) {
        if(pClear->IsEmpty()) {
            m_pTable->Clear(TRUE);
            return colCountInResult;
        } else if(1 == (long)pClear->GetNumeric())
            m_pTable->Clear(FALSE);
        int colInTables = m_pTable->GetColumnCount();
...
class IVLResultLoader : public ISQLiteResultLoader {
  public:
    ...
    virtual int init(int colCountInResult, sqlite3*, CValue* pParam) {
        int param = 0;
    ...
    // Добавление колонки в результат запроса
    virtual void setColumn(int, const CString&, const CType&) {
    }
    // Добавление строки в результат запроса.
    // Параметр - массив указателей на CValue, количеством равный количеству
    // добавленных колонок
...
class IScalarResultLoader : public ISQLiteResultLoader {
  public:
    virtual int init(int, sqlite3*, CValue*) {
        return m_column + 1;
    }
    // Добавление колонки в результат запроса
    virtual void setColumn(int, const CString&, const CType&) {
