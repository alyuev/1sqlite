...
struct one_field {
    ...
    BOOL isField() const {
        return 0 != (type & (fField|fNumNegateField));
    }
    BOOL isIndex() const {
        return 0 != (type & fVirtIdx);
    }
    BOOL isLnStr() const {
        return 0 != (type & fLongStr);
    }
    DWORD pos() const {
        return position;
    }
...
class CVtabInfo : public sqlite3_vtab {
  public:
    CTableEx* __fastcall table() const {
        return m_pTable;
    }
    const CString& __fastcall tableName() const {
        return m_strTableName;
    }
    const CString& __fastcall Name() const {
        return m_str1CTableName;
    }
    const CString& __fastcall textSqlCreate() const {
        return m_strSqlCreate;
    }
    const phisical_info& __fastcall phisInfo() const {
        return *m_phisInfo;
    }
    const one_field& __fastcall field(DWORD idx) const {
        return m_pFields[idx];
    }
    static const CString& __fastcall lastError() {
        return m_lastError;
    }

    int __fastcall bestIndex(sqlite3_index_info* pIdx) const;

    void __fastcall beginRead(record_bufer& bufer) const {
        m_tableReader.connectReader(bufer, m_pTable, m_phisInfo->tableReadBufer(), m_phisInfo->recordSize());
    }
    void __fastcall doneRead(record_bufer& bufer) const {
        m_tableReader.disconnectReader(bufer, m_pTable, m_phisInfo->tableReadBufer());
    }
    void __fastcall storeRecord(cursor_data& data) const {
        m_tableReader.read(data.record, m_phisInfo->tableReadBufer(), m_phisInfo->recordSize());
    }

    int __fastcall openCursor(sqlite3_vtab_cursor **ppCursor);
    // Получить экземпляр CVtabInfo по переданному пользовательскому имени
    // (Справочник.ТакойТо и тп)
    static CVtabInfo* __fastcall tabInfoForName(const CString& strUserName);
    static void __fastcall doneWork();
  ...
  protected:
    ...
    static void __fastcall fillNamesFromObjs(CMetaDataObjArray* arr, CNoCaseMap<CString>& aliaces, CDWordArray* longStr = NULL);
    friend struct tab_deleter;


    void __fastcall fillTabInfo(CNoCaseMap<CString>& aliaces, CDWordArray* longStr = NULL);

    static CNoCaseMap<CVtabInfo*> m_allTabs;
    static CString m_lastError;

    CString m_str1CTableName;
...
class CursorImpl : public sqlite3_vtab_cursor {
  public:
    ...
    int __fastcall RowID(sqlite3_int64 *pRowid) {
        if(m_pIterator) {
    ...
    int __fastcall Filter(int idxNum, const char *str, int, sqlite3_value **argv) {
    ...
    int __fastcall Column(sqlite3_context* pCtx, int nCol) {
  ...
  protected:
    ...
    const CVtabInfo& __fastcall  table() {
        return *static_cast<CVtabInfo*>(pVtab);
    }
    cursor_data m_data;
