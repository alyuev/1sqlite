...
BOOL StrategyCash::getFromCash(sqlite3_index_info* pIdx, CString& cashKey) {
    ...
    pIdx->estimatedCost = node->estimatedCost;
    pIdx->estimatedRows = node->estimatedRows;
    pIdx->idxFlags = node->idxFlags;
...
void StrategyCash::addToCash(const CString& cashKey, sqlite3_index_info* pIdx) {
    ...
    node->estimatedCost = pIdx->estimatedCost;
    node->estimatedRows = pIdx->estimatedRows;
    node->idxFlags = pIdx->idxFlags;
