...
void CValue_SetEmptyDefaultParam(CValue* pVal)
{
	if (pVal)
	{
		pVal->Reset();
		pVal->SetTypeCode(EMPTY_DEFAULT_PARAM_TYPE);
	}
};
...
void GetRTCFull(const CRuntimeClass* rtc, CString& Str)
{
    if (rtc)
    {
...
void MessageFullRTC(const CObject* obj, CString Indent/*=""*/)
{
        if (obj)
			MessageFullRTC(obj->GetRuntimeClass(), Indent);
}

void MessageFullRTC(const CRuntimeClass* rtc, CString Indent/*=""*/)
{
        if (rtc)
        {
                CString Str;
...
void TraceWindows(CWnd* pWnd, BOOL bWithMsgMap, UINT nMessageFilter, UINT nIDFilter, CString Indent)
{
		...
		if (pWnd->IsKindOf(RUNTIME_CLASS(CView)))
		{
			...
			if (pView->GetDocument()->GetDocTemplate())
			{
				Str.Empty();
				GetRTCFull(pView->GetDocument()->GetDocTemplate()->GetRuntimeClass(), Str);
				CTemplate7::GetDocumentID(pView->GetDocument());
				DoMsgLine("%s(Template:%s; ID:%X; tmplID:%X)", mmNone, Indent, Str,
					CTemplate7::GetDocumentID(pView->GetDocument()),
					CTemplate7::GetDocumentTemplateID(pView->GetDocument()));
			}
	...
	if (pMSGMAP)
	{
		...
		while (lpEntries->nMessage)
		{
			if (!nMessageFilter || nMessageFilter == lpEntries->nMessage)
			{
				...
				{
					DoMsgLine("%sMessage:%08X Code:%08X ID:%08X LastID:%08X Sig:%08X pfn:%08X", mmNone, Indent,
						lpEntries->nMessage, lpEntries->nCode, lpEntries->nID, lpEntries->nLastID, lpEntries->nSig, lpEntries->pfn);
				}
			}
			lpEntries++;
		}
...
void FormatMessageAndRuntimeError(DWORD dwMessageId)
{
	...
	CString strErr;
	if (res)
		strErr.Format("0x%08X : %s", dwMessageId, (LPTSTR)lpMsgBuf);
